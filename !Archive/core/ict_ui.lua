...
function iction.createBottomBarArtwork()
    ...
    iction.uiBotBarArt = {}
    --- Fetch the skin table
    local skin = iction.skinData[iction.skin]
    print("Setting skin to skin: " .. tostring(iction.skin))

    --- Build the artwork
    for x = 1, iction.tablelength(skin) do
        skin[x]["uiParentFrame"] = iction.mainFrameBldr.frame
        skin[x]["point"]['p'] = iction.mainFrameBldr.frame
        local skinFrameBldr = iction.UIFrameElement
        iction.skinFrame = skinFrameBldr.create(skinFrameBldr, skin[x])
...
function iction.createBuffFrame()
    ...
    buffData['point']['p'] = iction.mainFrameBldr.frame
    buffData['w'] = fw
    buffData['h'] = fh
...
function iction.createDebuffColumns()
    ...
    for i = 1, 4, 1 do
        ...
        local colData = iction.ictDeBuffColumnData
              ...
              colData["point"]["p"] = iction.mainFrameBldr.frame
              colData["point"]["x"] = x
              colData["point"]["y"] = y

        local debuffColumnBldr = iction.UIFrameElement
...
function iction.createShardFrame()
    ...
    shardData["point"]["p"] = iction.mainFrameBldr.frame
    iction.shardFrameBldr = iction.UIFrameElement
...
function iction.createConflagFrame()
    ...
    conflagData["point"]["p"] = iction.mainFrameBldr.frame
    iction.conflagFrameBldr = iction.UIFrameElement
...
function iction.createInsanityFrame()
    ...
    insanityData["point"]["p"] = iction.mainFrameBldr.frame
    iction.insanityFrameBldr = iction.UIFrameElement
...
function iction.createArtifactFrame()
    ...
    if next(artifact) ~= nil then
        ...
        artifactData["point"]["p"] = iction.mainFrameBldr.frame

        iction.artifactFrameBldr = iction.UIFrameElement
...
function iction.createHighlightFrame()
    ...
    highlightData["w"] = fw
    highlightData["h"] = fh
    highlightData["point"]["p"] = iction.mainFrameBldr.frame
    iction.highlightFrameBldr = iction.UIFrameElement
    iction.highlightFrame = iction.highlightFrameBldr.create(iction.highlightFrameBldr, highlightData)
    local bghl = iction.highlightFrameBldr.addTexture(iction.highlightFrameBldr, "ict_highlightTexture", 1, 1, "BACKGROUND", true, nil, nil, nil, "Interface\\ChatFrame\\ChatFrameBackground", .1, .6, .1, 0)
    return bghl
end
...
function iction.createSWDFrame()
    ...
    SWDData["point"]["p"] = iction.mainFrameBldr.frame
    iction.SWDFrameBldr = iction.UIFrameElement
...
function iction.createVoidFrame()
    ...
    VFData["point"]["p"] = iction.mainFrameBldr.frame
    iction.voidFrameBldr = iction.UIFrameElement
